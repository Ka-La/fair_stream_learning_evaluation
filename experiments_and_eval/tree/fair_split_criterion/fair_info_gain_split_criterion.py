...
class FairInfoGainSplitCriterion(SplitCriterion):
    ...
    def __init__(self, min_branch_fraction, weigh=True):
        super().__init__()
        self.min_branch_fraction = min_branch_fraction
        self.weigh_discrimination = weigh
        self.normalize_discrimination = False
    ...
    def merit_of_split(self, pre_split_dist, post_split_dist, attr_name, post_disc_merit, node, sens_att_name, trade_off):
        ...
        if (post_disc_merit == -1):
            ...
            #this is frequently > 1, as it is not normalized


        disc_loss = pre_disc - post_disc
        #disc_loss is frequently very negative



        if (disc_loss > 0.0):
            fair_info_gain = info_gain * disc_loss
        else:
            fair_info_gain = info_gain

        
        #fair_info_gain = info_gain * disc_loss #using this always will lead to better results and is done in code but not described by paper
