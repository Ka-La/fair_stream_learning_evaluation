...
from .fair_split_criterion import FairEnhancingInfoGainSplitCriterion, FairInfoGainSplitCriterion, FlexibleInfoGainSplitCriterion
from .fair_splitter import GaussianSplitter, Splitter, NominalSplitterClassif
...
class HoeffdingTreeClassifier(HoeffdingTree, base.Classifier):
    ...
    _FLEXIBLE_INFO_GAIN_SPLIT = "flex_info_gain"
    _FAIR_INFO_GAIN_SPLIT = "fair_info_gain"
    _FAIR_ENHANCING_INFO_GAIN_SPLIT = "fair_enhancing_info_gain"
    _VALID_SPLIT_CRITERIA = [_FAIR_ENHANCING_INFO_GAIN_SPLIT, _FAIR_INFO_GAIN_SPLIT, _FLEXIBLE_INFO_GAIN_SPLIT]
    ...
    def __init__(
        ...
        split_criterion: str = "fair_info_gain",
        delta: float = 1e-7,
        tau: float = 0.05,
        ...
        self.deprived_idx = deprived_idx
        ...
        if splitter is None:
            self.splitter = GaussianSplitter(self.deprived_idx)
        else:
        ...
        if split_criterion not in self._VALID_SPLIT_CRITERIA:
            print(
                "Invalid split_criterion option {}', will use default '{}'".format(
                    split_criterion, self._FAIR_INFO_GAIN_SPLIT
                )
            )
            self._split_criterion = self._FAIR_INFO_GAIN_SPLIT
        else:
    ...
    def _new_leaf(self, initial_stats=None, parent=None):
        ...
        if self._leaf_prediction == self._MAJORITY_CLASS:
            return LeafMajorityClass(initial_stats, depth, self.splitter, self.sensitive_attribute, self.deprived_idx, self.trade_off)
        elif self._leaf_prediction == self._NAIVE_BAYES:
            return LeafNaiveBayes(initial_stats, depth, self.splitter, self.sensitive_attribute, self.deprived_idx, self.trade_off)
        else:  # Naives Bayes Adaptive (default)
            return LeafNaiveBayesAdaptive(initial_stats, depth, self.splitter, self.sensitive_attribute, self.deprived_idx, self.trade_off)

    def _new_split_criterion(self):
        if self._split_criterion == self._FAIR_INFO_GAIN_SPLIT:
            split_criterion = FairInfoGainSplitCriterion(self.min_branch_fraction)
        elif self._split_criterion == self._FAIR_ENHANCING_INFO_GAIN_SPLIT:
            split_criterion = FairEnhancingInfoGainSplitCriterion(self.min_branch_fraction)
        elif self._split_criterion == self._FLEXIBLE_INFO_GAIN_SPLIT:
            split_criterion = FlexibleInfoGainSplitCriterion(self.min_branch_fraction)
        else:
