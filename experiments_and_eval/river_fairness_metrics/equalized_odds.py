...
class Equalized_FPR(metrics.base.Metric):
    ...
    def __init__(self, protected_attribute, time_decay = 0):
        ...
        self.sensitive_false_pos_number = 0
        self.sensitive_pos_number = 0
        self.sensitive_neg_number = 0
        
        self.control_true_pos_number = 0
        self.control_false_pos_number = 0
        self.control_pos_number = 0
        self.control_neg_number = 0
    ...
    def update(self, x, y_true, y_pred):
        ...
        self.sensitive_false_pos_number *= (1.0-self.time_decay)
        self.sensitive_pos_number *= (1.0-self.time_decay)
        self.sensitive_neg_number *= (1.0-self.time_decay)
        self.control_true_pos_number *= (1.0-self.time_decay)
        self.control_false_pos_number *= (1.0-self.time_decay)
        self.control_pos_number *= (1.0-self.time_decay)
        self.control_neg_number *= (1.0-self.time_decay)
        ...
        if x[self.sensitive_feature] == self.sensitive_value:
            if(float(y_true) == 1.0):
                ...
                self.sensitive_false_pos_number += float(y_pred)
                self.sensitive_neg_number += 1
        else:
            if(float(y_true) == 1.0):
                self.control_true_pos_number += float(y_pred)
                self.control_pos_number += 1
            else:
                self.control_false_pos_number += float(y_pred)
                self.control_neg_number += 1
        
    def get(self):
        try:
            sensitive_tpr = self.sensitive_true_pos_number/self.sensitive_pos_number
            sensitive_fpr = self.sensitive_false_pos_number/self.sensitive_neg_number

            control_tpr = self.control_true_pos_number/self.control_pos_number
            control_fpr = self.control_false_pos_number/self.control_neg_number

            tpr_diff = abs(control_tpr - sensitive_tpr)
            fpr_diff = abs(control_fpr - sensitive_fpr)
