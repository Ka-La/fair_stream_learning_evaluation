...
def _progressive_ind_fair_validation(
    ...
    sensitive_feature, sensitive_value = protected_attribute
    ...
    for i, x, y, *kwargs in stream.simulate_qa(dataset, moment, delay, copy=True):
        ...
        #Create x_opp as x with flipped sensitive feature
        if x[sensitive_feature] == sensitive_value:
            x_opp = x.copy()
            x_opp[sensitive_feature] = np.random.choice(other_attributes)

        else:
            x_opp = x.copy()
            x_opp[sensitive_feature] == sensitive_value
        ...
        if y is None:
            ...
            y_opp_pred = pred_func(x_opp, **kwargs)
            ...
            if utils.inspect.isanomalyfilter(model):
                ...
                y_opp_pred = model.classify(y_opp_pred)
            preds[i] = y_pred, ask_for_label
            opp_preds[i] = y_opp_pred
            continue

        # Case 2: there's a ground truth, model and metric can be updated
        y_pred, use_label = preds.pop(i)
        y_opp_pred = opp_preds.pop(i)

        # Update the metric
        if y_pred != {} and y_opp_pred != {} and y_pred is not None and y_opp_pred is not None:
            metric.update(y_opp_pred=y_opp_pred, y_pred=y_pred, x=x)
...
def _progressive_validation(
    ...
    for i, x, y, *kwargs in stream.simulate_qa(dataset, moment, delay, copy=True):
        ...
        if y_pred != {} and y_pred is not None:
            metric.update(y_true=y, y_pred=y_pred, x=x)
...
def iter_progressive_val_score(
    ...
    delay: str | int | dt.timedelta | typing.Callable | None = None,
    step=1,
    ind_fairness = False, #added params for individual fairness testing
    ...
    if not ind_fairness:
        ...
        yield from _progressive_ind_fair_validation(
            dataset,
            model,
            metric,
            checkpoints=itertools.count(step, step) if step else iter([]),
