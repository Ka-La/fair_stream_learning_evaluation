...
def greedy_bayes(dataset: DataFrame, k: int, epsilon: float, seed=0, set_root_attribute=None):
    ...
    if(set_root_attribute is not None):
        root_attribute = set_root_attribute
    else:
...
def get_noisy_distribution_of_attributes(attributes, encoded_dataset, epsilon=0.1):
    print("noisy distribution building")
...
def construct_noisy_conditional_distributions(bayesian_network, encoded_dataset, epsilon=0.1):
    ...
    k = 0# len(bayesian_network[-1][1])
    conditional_distributions = {}

    # first k+1 attributes
    roots = bayesian_network[0][1]#[0]
    kplus1_attributes = roots #[root]
...
=======
        print("Child: " + str(child) + ", Parents: " + str(parents))
>>>>>>> d1c6c754d8075adca1901ba4d9ce6379f9b003c9
        ...
        for parents_instance, stats_sub in stats.groupby(parents_grouper):
            ...
            print("Parent done: " + str(parents_instance))
